...
*/
static int64_t get_metadata_value(sqlite3 *db, const char *index_name,
                                  const char *key) {
  char *sql = sqlite3_mprintf("SELECT value FROM %s_metadata WHERE key = %Q",
                              index_name, key);
  if (!sql)
    return -1;
...
void test_clear_index_preserves_metadata(void) {
  ...
  int64_t max_neighbors =
      get_metadata_value(db, "test_preserve", "max_neighbors");
