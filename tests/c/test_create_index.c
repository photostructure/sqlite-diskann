...
void test_create_index_zero_dimensions(void) {
  ...
  DiskAnnConfig config = {.dimensions = 0, /* Invalid! */
                          .metric = DISKANN_METRIC_EUCLIDEAN};
...
void test_shadow_table_schema(void) {
  ...
  sqlite3_stmt *stmt;
  rc = sqlite3_prepare_v2(db, "PRAGMA table_info(test_index_shadow)", -1, &stmt,
                          NULL);
  TEST_ASSERT_EQUAL(SQLITE_OK, rc);
...
void test_metadata_roundtrip(void) {
  ...
  DiskAnnConfig config = {.dimensions = 512,
...
void test_create_index_duplicate_fails(void) {
  ...
  /* Second create with different dimensions should also fail (not silently
   * overwrite) */
